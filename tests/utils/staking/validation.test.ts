...
} from "../../../src/utils/staking/validation";
import { testingNetworks } from "../../helper";

describe.each(testingNetworks)("validateStakingTxInputData", ({ datagen }) => {
  describe.each(Object.values(datagen))(
    ...
    (dataGenerator) => {
      ...
      const balance = dataGenerator.getRandomIntegerBetween(
        ...
        params.maxStakingAmountSat + 100000000,
      );
...
describe("validateStakingExpansionInputs", () => {
  ...
  const { stakingAmountSat, finalityProviderPksNoCoordHex, timelock, utxos } =
    ...
    expect(() =>
      validateStakingExpansionInputs({
        ...
        previousStakingInput: {
          finalityProviderPksNoCoordHex: finalityProviderPksNoCoordHex.slice(
            0,
            1,
          ),
          stakingAmountSat,
  ...
  it("should throw an error if the previous staking has more finality providers than the current staking", () => {
    const extraFp =
      stakingDatagen.generateRandomFidelityProviderPksNoCoordHex(1);
    expect(() =>
      validateStakingExpansionInputs({
        babylonBtcTipHeight: 1,
        inputUTXOs: utxos,
...
describe.each(testingNetworks)(
  ...
  ({ datagen: { stakingDatagen } }) => {
    ...
    const { covenantQuorum: requiredQuorum } = previousParams;
    ...
    it("should throw an error if the previous staking has less covenant members than the required quorum", () => {
      ...
      const newCovenantNoCoordPks = stakingDatagen
        .generateRandomCovenantCommittee(requiredQuorum)
        .map((buffer) => buffer.toString("hex"));
      const { covenantNoCoordPks } = newParams;

      newParams.covenantNoCoordPks = covenantNoCoordPks
        .slice(requiredQuorum, covenantNoCoordPks.length)
    ...
    it("should pass with number of rotated out covenant members less than the required quorum", () => {
      ...
      const newParams = JSON.parse(JSON.stringify(previousParams));
      const newCovenantNoCoordPks = stakingDatagen
        .generateRandomCovenantCommittee(requiredQuorum - 1)
        .map((buffer) => buffer.toString("hex"));
      const { covenantNoCoordPks } = newParams;

      newParams.covenantNoCoordPks = covenantNoCoordPks
        .slice(0, requiredQuorum)
        .concat(newCovenantNoCoordPks);
