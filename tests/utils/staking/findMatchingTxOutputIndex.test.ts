...
describe.each(testingNetworks)(
  ...
  ({ network, networkName, datagen: { stakingDatagen: dataGenerator } }) => {
    it(`${networkName} should find the correct output index for a valid address`, () => {
      ...
      const outputAddress1 = dataGenerator.getAddressAndScriptPubKey(
        keyPair1.publicKey,
      ).nativeSegwit.address;
      const outputAddress2 = dataGenerator.getAddressAndScriptPubKey(
        keyPair2.publicKey,
      ).nativeSegwit.address;

      // Add outputs to the transaction
      tx.addOutput(address.toOutputScript(outputAddress1, network), 1000000);
      tx.addOutput(address.toOutputScript(outputAddress2, network), 2000000);
    ...
    it(`${networkName} should handle no matching address from output scripts`, () => {
      ...
      tx.addOutput(Buffer.from("OP_RETURN xyz"), 1000000);
