...
describe("address type", () => {
  describe.each(testingNetworks)(
    "should return true for a valid address type",
    ({ network, datagen: { stakingDatagen: dataGenerator } }) => {
      ...
      it("should return true for a valid Native SegWit address", () => {
        expect(isNativeSegwit(addresses.nativeSegwit.address, network)).toBe(
          true,
        );
      });
  ...
  envNetworks.forEach(({ mainnetDatagen, signetDatagen }) => {
    ...
    it("should return false for a mis-matched address type in different networks", () => {
      expect(
        isTaproot(signetAddresses.nativeSegwit.address, networks.testnet),
      ).toBe(false);
      expect(
        isNativeSegwit(mainnetAddresses.taproot.address, networks.bitcoin),
      ).toBe(false);
    ...
    it("should return false for an incorrect network", () => {
      expect(
        isTaproot(mainnetAddresses.taproot.address, networks.testnet),
      ).toBe(false);
      expect(
        isTaproot(mainnetAddresses.taproot.address, networks.regtest),
      ).toBe(false);

      expect(isTaproot(signetAddresses.taproot.address, networks.bitcoin)).toBe(
        false,
      );

      expect(
        isNativeSegwit(mainnetAddresses.nativeSegwit.address, networks.testnet),
      ).toBe(false);
      expect(
        isNativeSegwit(mainnetAddresses.nativeSegwit.address, networks.regtest),
      ).toBe(false);

      expect(
        isNativeSegwit(signetAddresses.nativeSegwit.address, networks.bitcoin),
      ).toBe(false);
    });
  });
});
  ...
  ({ datagen: { stakingDatagen: dataGenerator } }) => {
    ...
    describe("getPublicKeyNoCoord", () => {
      ...
      it("should throw an error for an invalid public key", () => {
        const invalidPublicKey = "invalid_public_key";
        expect(() => getPublicKeyNoCoord(invalidPublicKey)).toThrow(
          "Invalid public key without coordinate",
        );
      });
    });
  },
...
describe.each(testingNetworks)(
  ...
  ({ network, datagen: { stakingDatagen: dataGenerator } }) => {
    ...
    const { timelock, stakerInfo, params } =
    ...
    describe("should derive the staking output address from the scripts", () => {
      ...
      const { outputAddress } = deriveStakingOutputInfo(scripts, network);
    ...
    it("should throw an error if no address available from creation of pay-2-taproot output", () => {
      ...
      const scripts = staking.buildScripts();
      expect(() => deriveStakingOutputInfo(scripts, network)).toThrow(
    ...
    it("should throw an error if fail to create pay-2-taproot output", () => {
      ...
      const scripts = staking.buildScripts();
      expect(() => deriveStakingOutputInfo(scripts, network)).toThrow("oops");
    });
  },
);
...
describe("transactionIdToHash", () => {
  ...
  describe.each(testingNetworks)(
    "should correctly convert transaction id to hash",
    ({ datagen: { stakingDatagen: dataGenerator } }) => {
      ...
      it("should throw an error if the transaction id is empty", () => {
        const txId = "";
        expect(() => transactionIdToHash(txId)).toThrow(
