...
describe.each(testingNetworks)(
  ...
  ({ network, networkName, datagen }) => {
    describe.each(Object.values(datagen))("stakingPsbt", (dataGenerator) => {
    ...
    const validateCommonFields = (
      ...
      expect(inputAmount - outputAmount - estimatedFee).toBeLessThan(
        BTC_DUST_SAT,
      );
      ...
      if (inputAmount - (randomAmount + estimatedFee) > BTC_DUST_SAT) {
        const expectedChangeAmount =
          inputAmount - (randomAmount + estimatedFee);
        ...
        expect(
          psbt.txOutputs.find(
            (output) =>
              ...
              address.toOutputScript(changeAddress, network).toString("hex"),
          ),
        ).toBeDefined();
      }
