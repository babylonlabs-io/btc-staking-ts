...
describe.each(testingNetworks)(
  ...
  ({ network, networkName, datagen }) => {
    describe.each(Object.values(datagen))("unbondingPsbt", (dataGenerator) => {
      ...
      it(`${networkName} - should return a valid psbt result with correct inputs and outputs`, () => {
        ...
        expect(psbt.txOutputs[0].value).toEqual(
          randomAmount - params.unbondingFeeSat,
        );
      ...
      it(`${networkName} - should throw error if unbonding tx has more than one output`, () => {
        ...
        invalidUnbondingTx.addOutput(tx.transaction.outs[0].script, 1000);
