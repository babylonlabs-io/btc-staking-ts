      ...
      (dataGenerator) => {
        ...
        describe("Error path", () => {
          it(`${networkName} - should throw an error if the public key is invalid`, () => {
            const { transaction: stakingExpansionTx } =
          ...
          it(`${networkName} - should throw an error if previous staking output is not P2TR`, () => {
            ...
            const modifiedPreviousTx = Transaction.fromBuffer(
              previousStakingTx.toBuffer(),
            );
          ...
          it(`${networkName} - should throw an error if staking expansion transaction doesn't have exactly 2 inputs`, () => {
            ...
            const modifiedExpansionTx = Transaction.fromBuffer(
              stakingExpansionTx.toBuffer(),
            );
          ...
          it(`${networkName} - should throw an error if previous staking input hash doesn't match`, () => {
            ...
            modifiedExpansionTx.ins[0].hash = Buffer.from(
              "0".repeat(64),
              "hex",
            );
        ...
        describe("Happy path", () => {
          it(`${networkName} - should return a valid PSBT with correct inputs and outputs`, () => {
            const { transaction: stakingExpansionTx, fundingUTXO } =
              stakingExpansionTransaction(
                network,
                previousStakingScript,
                stakingAmountSat,
            ...
            expect(psbt.data.inputs[0].witnessUtxo?.value).toEqual(
              stakingAmountSat,
            );
            ...
            expect(psbt.txInputs[1].hash).toEqual(
              transactionIdToHash(fundingUTXO.txid),
            );
