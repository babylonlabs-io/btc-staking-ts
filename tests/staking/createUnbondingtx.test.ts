...
describe.each(testingNetworks)(
  ...
  ({ network, networkName, datagen: { stakingDatagen: dataGenerator } }) => {
    ...
    it(`${networkName} should throw an error if fail to build scripts`, async () => {
      ...
      expect(() => staking.createUnbondingTransaction(stakingTx)).toThrow(
        "build script error",
      );
    });
    ...
    it(`${networkName} should successfully create an unbonding transaction & psbt`, async () => {
      ...
      expect(transaction.outs[0].value).toEqual(
        stakingAmountSat - params.unbondingFeeSat,
      );
      expect(psbt.txOutputs[0].value).toEqual(
        stakingAmountSat - params.unbondingFeeSat,
      );
