...
describe.each(testingNetworks)(
  ...
  ({ network, networkName, datagen: { stakingDatagen: dataGenerator } }) => {
    const feeRate = 1;
    const { stakingTx, stakerInfo, stakingInstance } =
    ...
    describe("Create withdraw early unbonded transaction", () => {
      it(`${networkName} should throw an error if fail to build scripts`, () => {
        jest
          ...
          .mockImplementation(() => {
            throw new Error(
              "withdraw early unbonded delegation build script error",
            );
          });

        expect(() =>
          stakingInstance.createWithdrawEarlyUnbondedTransaction(
            unbondingTx,
            feeRate,
          ),
      ...
      it(`${networkName} should throw an error if fail to build early unbonded withdraw tx`, () => {
        jest
          .spyOn(transaction, "withdrawEarlyUnbondedTransaction")
          .mockImplementation(() => {
      ...
      it(`${networkName} should create withdraw early unbonded transaction`, () => {
        ...
        expect(withdrawTx.psbt.txInputs[0].hash.toString("hex")).toBe(
    ...
    describe("Create timelock unbonded transaction", () => {
      it(`${networkName} should throw an error if fail to build scripts`, async () => {
        jest
          ...
          .mockImplementation(() => {
            throw new Error(
              "withdraw timelock unbonded delegation build script error",
            );
          });
        expect(() =>
          stakingInstance.createWithdrawStakingExpiredPsbt(stakingTx, feeRate),
      ...
      it(`${networkName} should throw an error if fail to build timelock unbonded withdraw tx`, async () => {
        jest
          .spyOn(transaction, "withdrawTimelockUnbondedTransaction")
          .mockImplementation(() => {
            throw new Error("fail to build withdraw tx");
          });
