...
describe.each(testingNetworks)(
  "Create slashing transactions",
  ({ network, networkName, datagen: { stakingDatagen: dataGenerator } }) => {
    const { stakingTx, stakingInstance, stakerInfo, params, stakingAmountSat } =
      dataGenerator.generateRandomStakingTransaction(network, 1);
    ...
    describe("Create slash early unbonded transaction", () => {
      it(`${networkName} should throw an error if fail to build scripts`, () => {
        jest
          .spyOn(stakingScript, "StakingScriptData")
          .mockImplementation(() => {
            throw new Error(
              "slash early unbonded delegation build script error",
            );
          });
      ...
      it(`${networkName} should throw an error if fail to build early unbonded slash tx`, () => {
        jest
          .spyOn(transaction, "slashEarlyUnbondedTransaction")
          .mockImplementation(() => {
      ...
      it(`${networkName} should create slash early unbonded transaction`, () => {
        ...
        expect(slashTx.psbt.txInputs[0].hash.toString("hex")).toBe(
          unbondingTx.getHash().toString("hex"),
        );
        ...
        const slashAmount = Math.round(
          stakingAmountLeftInUnbondingTx * params.slashing!.slashingRate,
        );
        expect(slashTx.psbt.txOutputs[0].value).toBe(slashAmount);
        expect(
        ...
        const userFunds =
          stakingAmountLeftInUnbondingTx -
          slashAmount -
          params.slashing!.minSlashingTxFeeSat;
    ...
    describe("Create slash timelock unbonded transaction", () => {
      it(`${networkName} should throw an error if fail to build scripts`, async () => {
        jest
          ...
          .mockImplementation(() => {
            throw new Error(
              "slash timelock unbonded delegation build script error",
            );
          });
      ...
      it(`${networkName} should throw an error if fail to build timelock unbonded slash tx`, async () => {
        jest
          .spyOn(transaction, "slashTimelockUnbondedTransaction")
          .mockImplementation(() => {
            throw new Error("fail to build slash tx");
          });
      ...
      it(`${networkName} should create slash timelock unbonded transaction`, async () => {
        ...
        expect(slashTx.psbt.txInputs.length).toBe(1);
        expect(slashTx.psbt.txInputs[0].hash.toString("hex")).toBe(
          stakingTx.getHash().toString("hex"),
        );
        expect(slashTx.psbt.txInputs[0].index).toBe(0);
        // verify outputs
        expect(slashTx.psbt.txOutputs.length).toBe(2);
        // slash amount
        const slashAmount = Math.round(
          stakingAmountSat * params.slashing!.slashingRate,
        );
        expect(slashTx.psbt.txOutputs[0].value).toBe(slashAmount);
        expect(
          Buffer.from(slashTx.psbt.txOutputs[0].script).toString("hex"),
        ...
        expect(slashTx.psbt.txOutputs[1].address).toBe(address);
        const userFunds =
          stakingAmountSat - slashAmount - params.slashing!.minSlashingTxFeeSat;
        expect(slashTx.psbt.txOutputs[1].value).toBe(userFunds);
        expect(slashTx.psbt.locktime).toBe(0);
        expect(slashTx.psbt.version).toBe(2);
      });
