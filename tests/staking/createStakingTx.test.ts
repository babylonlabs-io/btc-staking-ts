  ...
  ({ network, networkName, datagen: { stakingDatagen: dataGenerator } }) => {
    ...
    beforeEach(() => {
      ...
      const { address, scriptPubKey } =
        dataGenerator.getAddressAndScriptPubKey(publicKey).taproot;
    ...
    it(`${networkName} throw StakingError if stakerInfo is incorrect`, async () => {
      ...
      expect(
        () =>
          new Staking(
            network,
            stakerInfoWithCoordPk,
            params,
            finalityProviderPksNoCoordHex,
            timelock,
          ),
      ).toThrow("Invalid staker public key");
      ...
      expect(
        () =>
          new Staking(
            network,
            stakerInfoWithInvalidAddress,
            params,
            finalityProviderPksNoCoordHex,
            timelock,
          ),
      ).toThrow("Invalid staker bitcoin address");
    });

    it(`${networkName} should throw an error if input data validation failed`, async () => {
      jest
        .spyOn(stakingUtils, "validateStakingTxInputData")
        .mockImplementation(() => {
      ...
      expect(() =>
        staking.createStakingTransaction(
          params.minStakingAmountSat,
          utxos,
          feeRate,
        ),
      ).toThrow(new StakingError(StakingErrorCode.INVALID_INPUT, "some error"));
    });
    ...
    it(`${networkName} should successfully create a staking transaction & psbt`, async () => {
      ...
      expect(psbt.data.inputs[0].witnessUtxo?.script.toString("hex")).toEqual(
        utxos[0].scriptPubKey,
      );
      ...
      const psbtInputAmount = psbt.data.inputs.reduce(
      ...
      const txInputAmount = transaction.ins.reduce((sum, input) => {
        const matchingUtxo = utxos.find(
          (utxo) =>
            transactionIdToHash(utxo.txid).toString("hex") ===
              input.hash.toString("hex") && utxo.vout === input.index,
        );
      ...
      if (psbtChangeAmount > BTC_DUST_SAT) {
        ...
        expect(lastTxOutput.script).toEqual(
          address.toOutputScript(stakerInfo.address, network),
        );
      }
