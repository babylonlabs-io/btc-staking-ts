...
// TODO: To be removed
describe.each([networks.bitcoin, networks.testnet])(
  "Observal - Create staking transaction",
  (network) => {
    ...
    beforeEach(() => {
      ...
      finalityProviderPksNoCoord =
        dataGenerator.generateRandomFidelityProviderPksNoCoordHex(1);
    ...
    it(`${networkName} should throw an error if input data validation failed`, async () => {
      ...
      expect(() =>
        observableStaking.createStakingTransaction(
    ...
    it(`${networkName} should throw an error if fail to build scripts`, async () => {
      jest
        .spyOn(observableStakingScriptData, "ObservableStakingScriptData")
        .mockImplementation(() => {
    ...
    it(`${networkName} should successfully create a observable staking transaction`, async () => {
      ...
      if (changeAmount > BTC_DUST_SAT) {
        expect(transaction.outs[transaction.outs.length - 1].value).toEqual(
          changeAmount,
        );
        expect(transaction.outs[transaction.outs.length - 1].script).toEqual(
