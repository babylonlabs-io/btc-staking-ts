...
describe.each(testingNetworks)(
  ...
  ({ network, datagen: { stakingDatagen: dataGenerator } }) => {
    describe("validateDelegationInputs", () => {
      ...
      const feeRate = 1;
      const { stakingTx, timelock, stakerInfo, finalityProviderPksNoCoordHex } =
      ...
      it("should throw an error if the output index is out of range", () => {
        jest
          .spyOn(utils, "findMatchingTxOutputIndex")
          .mockImplementation(() => {
    ...
    describe("validateParams", () => {
      ...
      it("should throw an error if covenant public keys are with coordinates", () => {
        const params = {
          ...validParams,
          covenantNoCoordPks: validParams.covenantNoCoordPks.map(
            (pk) => "02" + pk,
          ),
        };
        ...
        ).toThrow("Covenant public key should contains no coordinate");
      });

      it("should throw an error if covenant public keys are less than the quorum", () => {
        const params = {
          ...validParams,
          covenantQuorum: validParams.covenantNoCoordPks.length + 1,
        };
      ...
      it("should throw an error if max staking time is less than min staking time", () => {
        const params = {
          ...validParams,
          maxStakingTimeBlocks: validParams.minStakingTimeBlocks - 1,
        };
